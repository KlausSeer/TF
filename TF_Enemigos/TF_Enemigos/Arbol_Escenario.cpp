...
#include<ctime>
	...
	Arbol_Escenario::Arbol_Escenario() {
		this->root = new Node(1, new Escenario(2, 1), nullptr, nullptr, nullptr); // Prof o Nivel del nodo raiz es 1
		this->current = root;
	}
			...
			case 3: 
				if(_c>0){
					if(this->current->right_Child_Esc != nullptr){ //Si es que hay hijo, nos movemos al hijo
						...
						int prof = this->current->profundidad;
						this->current->right_Child_Esc= new Node(rof+1, new Escenario(3, prof+1), this->current, nullptr, nullptr);
						this->current = this->current->right_Child_Esc;
					}	
					_c-=1;
				}
